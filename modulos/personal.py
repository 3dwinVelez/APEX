...
class PersonalModule:
    ...
    CRUD completo de empleados, técnicos y administradores
    """
    ...
    def form_especifico(self, rol):
        ...
        txt_nombre = ft.TextField(label="Nombre Completo", border_radius=10)
        txt_doc = ft.TextField(label="Documento Identidad", border_radius=10)
        ...
        controles = [
            ft.Text(f"REGISTRO DE {rol.upper()}", size=20, weight="bold", text_align="center"),
            container_id,
            ft.Container(height=10),
            txt_nombre,
            txt_doc,
        ...
        def guardar_clic(e):
            ...
            datos = {
                ...
                "user": nuevo_id,
                "pass": txt_pass.value or "1234",  # Contraseña por defecto
                "rol": rol, 
                "id_interno": nuevo_id,
                "empresa": dd_empresa.value if dd_empresa else "APEX",
                "costo": float(txt_costo.value or 0) if txt_costo else 0,
                "salario": float(txt_salario.value or 0) if txt_salario else 0,
                "extra": float(txt_extra.value or 0) if txt_extra else 0
            }
            
            if self.db.registrar_usuario_full_pro(datos):
                self.page.snack_bar = ft.SnackBar(ft.Text("✅ Usuario creado exitosamente"), bgcolor="#1B5E20")
                self.page.snack_bar.open = True
                self.mostrar_maestro_personal()
            else:
                self.page.snack_bar = ft.SnackBar(ft.Text("❌ Error al crear usuario"), bgcolor="#B71C1C")
                self.page.snack_bar.open = True
                self.page.update()

        controles.append(ft.Container(height=10))
        controles.append(ft.ElevatedButton(
        ...
        form_card = ft.Container(
            content=ft.Column(controles, horizontal_alignment="stretch", spacing=15),
